...
import { MapPin, Users, Plus, Pencil, Trash2, X, Check, AlertTriangle, Loader2, Lock, Building2, Store, ListChecks } from "lucide-react";
...
export default function Organisasi({ user }: { user: AuthUser }) {
  ...
  const load = React.useCallback(async () => {
    try {
      const [a, e, c] = await Promise.all([api.getAreas(), api.getEmployees(), api.getClients()]);
      setAreas(a);
      setEmps(e);
      setClients(c);
      setErr(null);
  ...
  return (
    <div className="space-y-6">
      <div className="pb-2 border-b border-slate-100">
        ...
        <p className="text-slate-400 text-xs mt-0.5">Area &amp; Karyawan — fondasi hirarki distribusi/roadshow + custodian aset internal.</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          <ClientCard clients={clients} onChange={load} onErr={setErr} />
